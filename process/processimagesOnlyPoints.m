function [finalPosList, dotlocations, numpointconsensus, numdotlocations, ...
        ...
        processimagesOnlyPoints(experimentDir, experimentName, position, ...
        numRounds, numChannels, processedImages, varargin)
...
% for dot location 3. boolean for saving points 4. allowed diff 5. square
% root radius
%
...
% Author: Nico Pierson
    ...
    if numvarargs > 3
        if ~ischar(varargin{4}) 
            error('src:processpoints:WrongInput', ...
                'processimages pointsSavePath requires type boolean');
        end
    end
    if numvarargs > 4
    ...
    if numvarargs >= argsLimit
        if ~ismatrix(varargin{7})
            error('src:processimages:WrongInput', ...
                'processimages var beadPoints requires type matrix');
        end
    end
    % set defaults for optional inputs
    optargs = {6, 'exons', 'none', true, 1, [], []};
    % assign defaults
    optargs(1:numvarargs) = varargin;
    % Default Value of ref image is 1
    [sqrtradius, typedots, superresolve, pointsSaveDir, alloweddiff, channel, beadPoints] = optargs{:}; % beadPoints used to remove bead points
    ...
    [points] = processpoints(experimentDir, experimentName, position, numRounds, numChannels, ...
        sqrtradius, alloweddiff, processedImages, typedots, superresolve, pointsSaveDir, channel);
