function [hybIms, dapiIms, tformDapi, numHybCh, numZSlice] = aligndapiimagesRot(experimentName, ...
...
% Updated: 8/6/2019
    ...
    for folder = 1:length(folderArray)
        ...
        %imageName = ['MMStack_Pos' num2str(position) '.ome.tif'];
        listing = dir([experimentDir '\HybCycle_' num2str(folderArray(folder)) '\*MMStack_Pos' num2str(position) '.ome.tif']);
        if length(listing) == 1
            imageName = listing(1).name;
        else
            error 'Image names are ambiguous';
        ...
        if folder ~= 1 || ~isempty(dapiRefPath)% if not the first folder
            ...
            % otherwise
            tformDapi{folder} = grabtformRot(newIm, newImRef, initialRadius, numIterations);
