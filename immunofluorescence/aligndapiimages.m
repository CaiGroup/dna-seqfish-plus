function [hybIms, dapiIms, tformDapi, numHybCh, numZSlice] = aligndapiimages(experimentName, ...
...
% Updated: 8/6/2019
    ...
    [dapiRefPath, saveData, saveHybIms, divideIms, dim, numChRef] = optargs{:};
    ...
    if ~isempty(dapiRefPath)
        ...
        [allIms, sizeCDapi, sizeZ, ~, ~] = grabimseries(imagePath, position);
        dapiref = allIms{sizeCDapi};
    else
        sizeCDapi = numChRef;
    end
    ...
    for folder = 1:length(folderArray)
        ...
        [allIms, sizeC, sizeZ, ~, ~] = grabimseries(imagePath, position, sizeCDapi);
        %if sizeZ < 4
