% script used to get beads from the images
%
% add a script to get Hyb0 tform and apply the tform to the beads
...
fixedThreshold = [10000 20000 15000]; %fixedThreshold = [5000 12000 18000]; % ch1 %fixedThreshold = [10000 20000 15000]; % Can add threshold if already set
...
listSavePath = fullfile(saveDir, ['ref-points-ForBeadAlignment-pos' num2str(position) '-radial3d-initialBeads-raw-intensity.csv']);
...
numHyb = numBeadDapi -1;
%fixedThreshold = [];
fixedThreshold = [7000 12000 10000]; %fixedThreshold = []; %
[refPoints, fixedThreshold] = matchpointsims(beadImsTemp(1:numHyb), ...
fixedThreshold, typedotsFixed, processBeads, backradius, sliding, ...
[], searchradius, 0, pwd, 'log');
...
refPointsAligned = refPoints;
listSavePath = fullfile(saveDir, ['ref-points-ForBeadAlignment-pos' num2str(position) '-radial3d-finalBeads-raw-intensity.csv']);
fileID = fopen(listSavePath,'w');
fprintf(fileID, '%s,%s,%s,%s,%s\n', 'ch', 'x', 'y', 'z', 'int');
for ch = 1:3
    pointsSize = length(refPoints(ch).channels);
...
save(fullfile(saveDir, ['beadInitialRefPoints_chaTforms-pos' num2str(position) '.mat']), 'refPointsAligned', 'chaTform', 'refPoints', 'fixedThreshold', 'bead2dapiTform');
