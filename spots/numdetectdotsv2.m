function [numPoints, logImage, regMax] = numdetectdotsv2(image, threshold, logImage, regMax, varargin)
    ...
    if isempty(logImage)
        ...
        switch typeDetectDots
            ...
            case 'introns'
                ...
                logImage1 = rawImage;
                logImage = rawImage;
                logImage1(belowThresh) = 0;
                dilate = imdilate(logImage1,regMask);

                % create logical matrix of dotsLogical where 1 is the voxel's value is
                % greater than its neighbors
                dotsLogical = logImage1 > dilate; 
            case 'exons2d'
                
                %% Find Dots using Laplacian filter
                rawImage = double(image);
                logImageSlice = zeros(size(rawImage)); % initialize logFish
                for i=1:size(rawImage, 3) % for each z-slice, apply laplacian filter
                    logImageSlice(:,:,i) = logMask(rawImage(:,:,i));

                    regMax(:,:,i) = imregionalmax(logImageSlice(:,:,i)); % get regional maxima
                    % create logical matrix of dots that are a regional maxima and above
                    % the treshold value
                    dotsLogical(:,:,i) = regMax(:,:,i) & logImageSlice(:,:,i) > threshold;
                end
        ...
        % else use use inputs of logImage and regMax
        switch typeDetectDots
            case 'exons'
                dotsLogical = regMax & logImage > threshold; 
            case 'introns'
                regMask = true(3,3,3);
                regMask(2,2,2) = false;
                logImageUse = logImage;
                belowThresh = logImageUse < threshold;
                logImageUse(belowThresh) = 0;
                dilate = imdilate(logImageUse,regMask);
                dotsLogical = logImageUse > dilate; 
            case 'exons2d'
                for i=1:size(logImage, 3) % for each z-slice, apply laplacian filter
                    dotsLogical(:,:,i) = regMax(:,:,i) & logImage(:,:,i) > threshold;
                end
            otherwise
                error 'detectdots var typedetectdots invalid argument';
        end
